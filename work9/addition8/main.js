...
for (const usersListElement of usersList) {
    ...
    idElement.innerText = usersListElement.id

    const nameElement = document.createElement('div')
    nameElement.innerText = usersListElement.name

    const usernameElement = document.createElement('div')
    usernameElement.innerText = usersListElement.username

    const emailElement = document.createElement('div')
    emailElement.innerText = usersListElement.email

    const addressElement = document.createElement('div')
    for (const address1 of usersListElement.address) {
        const streetElement = document.createElement('div')
        streetElement.innerText = address1.street
        addressElement.appendChild(streetElement)

        const suiteElement = document.createElement('div')
        suiteElement.innerText = address1.suite
        addressElement.appendChild(suiteElement)

        const cityElement = document.createElement('div')
        cityElement.innerText = address1.city
        addressElement.appendChild(cityElement)

        const zipcodeElement = document.createElement('div')
        zipcodeElement.innerText = address1.zipcode
        addressElement.appendChild(zipcodeElement)

        const geoElement = document.createElement('div')
        for (const geoElements of usersListElement.address.geo) {
            const latElement = document.createElement('div')
            latElement.innerText = geoElements.lat
            geoElement.appendChild(latElement)

            const lngElement = document.createElement('div')
            lngElement.innerText = geoElements.lng
    ...
    phoneElement.innerText = usersListElement.phone

    const websiteElement = document.createElement('div')
    websiteElement.innerText = usersListElement.website

    const companyElement = document.createElement('div')
    for (const companyElements of usersListElement.company) {
        const name = document.createElement('div')
        name.innerText = companyElements.name
        companyElement.appendChild(name)

        const catchPhrase = document.createElement('div')
        catchPhrase.innerText = companyElements.catchPhrase
        companyElement.appendChild(catchPhrase)

        const bs = document.createElement('div')
        bs.innerText = companyElements.bs
        companyElement.appendChild(bs)
    }
